...
// Vertical position of the edge button: fraction of viewport height at the
// button center. One global value shared by all tabs.
export const EDGE_BUTTON_POS_KEY = '__edge_button_pos';

/**
 * Edge-peek button for normal web pages.
 * Reveals progressively as mouse approaches the right edge.
 * Draggable vertically along the edge; position persists via storage.
 * Returns a destroy() so the content script can hot-swap when the user
 * flips the `__side_button_enabled` preference.
 */
export function createEdgeButton(opts?: { initialPos?: number }): { destroy: () => void } {
  if (!document.body) {
    let inner: { destroy: () => void } | null = null;
    document.addEventListener('DOMContentLoaded', () => { inner = createEdgeButton(opts); }, { once: true });
  ...
  let posFrac = opts?.initialPos ?? 0.5;

  function applyPos(frac: number) {
    const half = H / 2 + PAD;
    const y = Math.min(window.innerHeight - half, Math.max(half, frac * window.innerHeight));
    posFrac = y / window.innerHeight;
    clip.style.top = `${y}px`;
    // Popup follows the button but its center stays clamped away from the
    // viewport edges so it doesn't overflow when the button is parked there.
    const py = Math.min(window.innerHeight - 120, Math.max(120, y));
    popupEl.style.top = `${py}px`;
  }
  applyPos(posFrac);

  const onResize = () => applyPos(posFrac);
  ...
  // Vertical drag along the edge. Pointer capture keeps the drag alive when
  // the cursor leaves the narrow clip container; a 4px threshold separates
  // drags from clicks.
  ...
  btn.addEventListener('pointerdown', (e) => {
    ...
    dragStartTop = posFrac * window.innerHeight;
  ...
  btn.addEventListener('pointermove', (e) => {
    ...
    applyPos((dragStartTop + dy) / window.innerHeight);
  });
  const endDrag = (e: PointerEvent) => {
    if (dragPointerId !== e.pointerId) return;
    dragPointerId = null;
    if (dragMoved) chrome.storage.local.set({ [EDGE_BUTTON_POS_KEY]: posFrac });
  };
  ...
  btn.addEventListener('click', (e) => {
    e.stopPropagation();
    if (dragMoved) return; // drag release, not a click
