...
`}var p=`__app_sources`,m=`__app_manifests`,h=`__dev_server_online`,g=12e3,_=8e3,v=[1e3,3e3],y=[500,1500],b=new URLSearchParams(window.location.search),x=b.get(`app`);x?C(x):document.getElementById(`loading`).textContent=`Missing app parameter`;function S(e){document.body.innerHTML=``;let t=`#b91c1c`,n=`#1c1917`,r=document.createElement(`div`);r.id=`airglow-app-offline`,r.style.cssText=`position:fixed;inset:0;display:flex;align-items:center;justify-content:center;padding:24px;background:#f5f5f4;color:${n};`,r.innerHTML=`
  ...
  `,document.body.appendChild(r),document.getElementById(`airglow-offline-retry`)?.addEventListener(`click`,()=>location.reload());let i=e=>{h in e&&e[h].newValue===!0&&(chrome.storage.local.onChanged.removeListener(i),location.reload())};chrome.storage.local.onChanged.addListener(i)}async function C(e){let t=await chrome.storage.local.get([h,p]),n=(t[p]||{})[e];if(t[h]===!1&&(!n||n.type===`local`)){S(e);return}if(n?.url){w(e,n);return}for(let t=0;t<5;t++){t>0&&await new Promise(e=>setTimeout(e,1e3));let n=((await chrome.storage.local.get(p))[p]||{})[e];if(n?.url){w(e,n);return}}u.warn(`airglow`,`no source registered for app '${e}' after 5 attempts`),document.getElementById(`loading`).textContent=`App '${e}' not found. Is an app source reachable?`}function w(t,n){let r=n.url.replace(/\/+$/,``),i=null,a=null,o=0,s=!1,c=null,l=``;chrome.storage.local.get(m).then(e=>{let n=(Array.isArray(e[m])?e[m]:[]).find(e=>e?.id===t);n?.name&&(document.title=n.name)}).catch(e=>{u.warn(`airglow`,`cached manifest title lookup failed: ${e instanceof Error?e.message:String(e)}`)});function d(e=!1){let n=new URLSearchParams(b);return e&&n.set(`_airglow_reload`,String(Date.now())),`${r}/api/apps/${t}/ui?${n.toString()}`}function p(e=!1){let n=new URLSearchParams;e&&n.set(`_airglow_reload`,String(Date.now()));let i=n.toString();return`${r}/api/apps/${t}/ui-bundle${i?`?${i}`:``}`}function h(e=!1){let n=new URLSearchParams(b);return n.set(`app`,t),n.set(`nonce`,l),e&&n.set(`_airglow_reload`,String(Date.now())),chrome.runtime.getURL(`app-ui-sandbox.html?${n.toString()}`)}function x(e){return new Promise(t=>setTimeout(t,e))}function S(e){return e===408||e===409||e===425||e===429||e>=500&&e<=599}async function C(t){let r=``,i=n.type===`cloud`?await e():{};for(let e=0;e<=y.length;e++)try{let n=await fetch(t,{headers:i,signal:AbortSignal.timeout(_)});if(!n.ok&&S(n.status)&&e<y.length){r=`UI bundle request failed with HTTP ${n.status}`,await x(y[e]);continue}return n}catch(t){if(r=t instanceof Error?t.message:String(t),e<y.length){await x(y[e]);continue}throw Error(`UI bundle request failed after ${y.length+1} attempts: ${r}`)}throw Error(`UI bundle request failed: ${r}`)}function w(e){let t=document.getElementById(`loading`);t||(t=document.createElement(`div`),t.id=`loading`,document.body.appendChild(t)),t.textContent=e}function E(){a&&=(clearTimeout(a),null)}function D(){document.getElementById(`airglow-app-crash`)?.remove(),s=!1}function O(e){return[e.name?`${e.name}: ${e.message}`:e.message,e.filename?`${e.filename}:${e.lineno??0}:${e.colno??0}`:``,e.stack||``].filter(Boolean).join(`

`)}function k(e,t,n){s=!0,E();let r=document.getElementById(`airglow-app-crash`);r&&r.remove();let i=document.createElement(`div`);i.id=`airglow-app-crash`,i.style.cssText=[`position:fixed`,`inset:0`,`z-index:2147483647`,`display:flex`,`align-items:center`,`justify-content:center`,`padding:24px`,`background:#f5f5f4`,`color:#1c1917`,`font-family:system-ui,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif`].join(`;`);let a=n?O(n):``;i.innerHTML=`
    ...
    `,document.body.appendChild(i),document.getElementById(`airglow-reload-app`)?.addEventListener(`click`,()=>j(!0)),document.getElementById(`airglow-reload-extension`)?.addEventListener(`click`,()=>chrome.runtime.reload())}function A(){E(),a=setTimeout(()=>{let e=v[o];if(e!==void 0){o++,w(`App did not load. Retrying ${o}/${v.length}...`),window.setTimeout(()=>j(!0),e);return}k(`App '${t}' did not finish loading`,`The UI iframe did not fire a load event. This is usually a dev server, network, or bundle problem. Reloading the app iframe is enough; you should not need to restart the whole extension.`)},g)}function j(e=!1){if(D(),w(`Loading...`),i){if(n.type===`cloud`){E(),M(e).catch(e=>{k(`App '${t}' failed to load`,e instanceof Error?e.message:String(e))});return}i.setAttribute(`sandbox`,`allow-scripts allow-same-origin allow-forms`),i.src=d(e),A()}}async function M(e=!1){if(!i)return;let n=await C(p(e));if(!n.ok){let e=`${n.status}`;try{let t=await n.json();t?.error&&(e=typeof t.error==`string`?t.error:JSON.stringify(t.error))}catch{}throw Error(`UI bundle request failed: ${e}`)}l=crypto.randomUUID(),c={sdk:f(t),code:await n.text()},i.setAttribute(`sandbox`,`allow-scripts allow-forms`),A(),i.src=h(e)}window.addEventListener(`message`,e=>{if(!i||e.source!==i.contentWindow)return;let n=e.source,r=e.data;if(r?._airglow_app_error&&r.appId===t){s||k(`App '${t}' hit an unhandled error`,`The app iframe is still isolated. You can reload only this app without restarting the extension.`,r);return}if(!r?._airglow)return;let a={...r,_appId:t};chrome.runtime.sendMessage(a,e=>{let t=chrome.runtime.lastError?{error:chrome.runtime.lastError.message||`Chrome runtime message failed`,code:`CHROME_RUNTIME_ERROR`}:e||{};n?.postMessage({_airglow_response:!0,_callId:r._callId,...t},`*`)})}),i=document.createElement(`iframe`),i.style.cssText=`position:fixed;inset:0;width:100%;height:100%;border:none;`,i.setAttribute(`sandbox`,`allow-scripts allow-same-origin allow-forms`),i.setAttribute(`allow`,`clipboard-read; clipboard-write`),i.onload=()=>{o=0,E(),document.getElementById(`loading`)?.remove(),c&&i?.contentWindow&&(i.contentWindow.postMessage({type:`airglow:ui:run`,appId:t,nonce:l,...c},`*`),c=null)},i.onerror=()=>{k(`App '${t}' failed to load`,`The browser reported an iframe load error. Reloading this app iframe should recover after a transient server or network issue.`)},document.body.appendChild(i),j(!1)}function T(e){let t=document.createElement(`div`);return t.textContent=e||``,t.innerHTML}