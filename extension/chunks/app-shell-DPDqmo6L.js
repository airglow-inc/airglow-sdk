...
`)}function O(e,t,n){o=!0,T();let r=document.getElementById(`airglow-app-crash`);r&&r.remove();let i=document.createElement(`div`);i.id=`airglow-app-crash`,i.style.cssText=[`position:fixed`,`inset:0`,`z-index:2147483647`,`display:flex`,`align-items:center`,`justify-content:center`,`padding:24px`,`background:#f5f5f4`,`color:#1c1917`,`font-family:system-ui,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif`].join(`;`);let a=n?D(n):``;i.innerHTML=`
    ...
    `,document.body.appendChild(i),document.getElementById(`airglow-reload-app`)?.addEventListener(`click`,()=>A(!0)),document.getElementById(`airglow-reload-extension`)?.addEventListener(`click`,()=>chrome.runtime.reload())}function k(){T(),i=setTimeout(()=>{let t=_[a];if(t!==void 0){a++,C(`App did not load. Retrying ${a}/${_.length}...`),window.setTimeout(()=>A(!0),t);return}O(`App '${e}' did not finish loading`,`The UI iframe did not fire a load event. This is usually a dev server, network, or bundle problem. Reloading the app iframe is enough; you should not need to restart the whole extension.`)},h)}function A(n=!1){if(E(),C(`Loading...`),r){if(t.type===`cloud`){T(),j(n).catch(t=>{O(`App '${e}' failed to load`,t instanceof Error?t.message:String(t))});return}r.setAttribute(`sandbox`,`allow-scripts allow-same-origin allow-forms`),r.src=u(n),k()}}async function j(t=!1){if(!r)return;let n=await S(f(t));if(!n.ok){let e=`${n.status}`;try{let t=await n.json();t?.error&&(e=typeof t.error==`string`?t.error:JSON.stringify(t.error))}catch{}throw Error(`UI bundle request failed: ${e}`)}c=crypto.randomUUID(),s={sdk:d(e,`app_ui`),code:await n.text()},r.setAttribute(`sandbox`,`allow-scripts allow-forms`),k(),r.src=m(t)}window.addEventListener(`message`,t=>{if(!r||t.source!==r.contentWindow)return;let n=t.source,i=t.data;if(i?._airglow_app_error&&i.appId===e){o||O(`App '${e}' hit an unhandled error`,`The app iframe is still isolated. You can reload only this app without restarting the extension.`,i);return}if(!i?._airglow)return;let a={...i,_appId:e};chrome.runtime.sendMessage(a,e=>{let t=chrome.runtime.lastError?{error:chrome.runtime.lastError.message||`Chrome runtime message failed`,code:`CHROME_RUNTIME_ERROR`}:e||{};n?.postMessage({_airglow_response:!0,_callId:i._callId,...t},`*`)})}),r=document.createElement(`iframe`),r.style.cssText=`position:fixed;inset:0;width:100%;height:100%;border:none;`,r.setAttribute(`sandbox`,`allow-scripts allow-same-origin allow-forms`),r.setAttribute(`allow`,`clipboard-read; clipboard-write`),r.onload=()=>{a=0,T(),document.getElementById(`loading`)?.remove(),s&&r?.contentWindow&&(r.contentWindow.postMessage({type:`airglow:ui:run`,appId:e,nonce:c,...s},`*`),s=null)},r.onerror=()=>{O(`App '${e}' failed to load`,`The browser reported an iframe load error. Reloading this app iframe should recover after a transient server or network issue.`)},document.body.appendChild(r),A(!1)}function w(e){let t=document.createElement(`div`);return t.textContent=e||``,t.innerHTML}