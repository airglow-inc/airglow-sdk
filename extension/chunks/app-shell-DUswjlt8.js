...
`}var p=`__app_sources`,m=`__app_manifests`,h=`__dev_server_online`,g=12e3,_=8e3,v=[1e3,3e3],y=[500,1500],b=`_airglowRuntimeUserApproved`,x=new URLSearchParams(window.location.search),S=x.get(`app`);S?w(S):document.getElementById(`loading`).textContent=`Missing app parameter`;function C(e){document.body.innerHTML=``;let t=`#b91c1c`,n=`#1c1917`,r=document.createElement(`div`);r.id=`airglow-app-offline`,r.style.cssText=`position:fixed;inset:0;display:flex;align-items:center;justify-content:center;padding:24px;background:#f5f5f4;color:${n};`,r.innerHTML=`
...
`)}function A(e,t,n){s=!0,D();let r=document.getElementById(`airglow-app-crash`);r&&r.remove();let i=document.createElement(`div`);i.id=`airglow-app-crash`,i.style.cssText=[`position:fixed`,`inset:0`,`z-index:2147483647`,`display:flex`,`align-items:center`,`justify-content:center`,`padding:24px`,`background:#f5f5f4`,`color:#1c1917`,`font-family:system-ui,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif`].join(`;`);let a=n?k(n):``;i.innerHTML=`
      <div style="width:min(640px,100%);border:1px solid #e7e5e4;border-radius:8px;background:#fff;padding:20px;box-shadow:0 12px 32px rgba(28,25,23,.12)">
        ...
        <div style="font-size:20px;font-weight:650;line-height:1.25;margin-bottom:8px">${E(e)}</div>
        <div style="font-size:14px;line-height:1.5;color:#57534e;margin-bottom:16px">${E(t)}</div>
        ${a?`<pre style="max-height:220px;overflow:auto;white-space:pre-wrap;background:#fafaf9;border:1px solid #e7e5e4;border-radius:6px;padding:12px;font-size:12px;line-height:1.45;color:#44403c;margin-bottom:16px">${E(a)}</pre>`:``}
    ...
    `,document.body.appendChild(i),document.getElementById(`airglow-reload-app`)?.addEventListener(`click`,()=>M(!0)),document.getElementById(`airglow-reload-extension`)?.addEventListener(`click`,()=>chrome.runtime.reload())}function j(){D(),a=setTimeout(()=>{let e=v[o];if(e!==void 0){o++,T(`App did not load. Retrying ${o}/${v.length}...`),window.setTimeout(()=>M(!0),e);return}A(`App '${t}' did not finish loading`,`The UI iframe did not fire a load event. This is usually a dev server, network, or bundle problem. Reloading the app iframe is enough; you should not need to restart the whole extension.`)},g)}function M(e=!1){if(O(),T(`Loading...`),i){if(n.type===`cloud`){D(),N(e).catch(e=>{A(`App '${t}' failed to load`,e instanceof Error?e.message:String(e))});return}i.setAttribute(`sandbox`,`allow-scripts allow-same-origin allow-forms`),i.src=d(e),j()}}async function N(e=!1){if(!i)return;let n=await w(p(e));if(!n.ok){let e=`${n.status}`;try{let t=await n.json();t?.error&&(e=typeof t.error==`string`?t.error:JSON.stringify(t.error))}catch{}throw Error(`UI bundle request failed: ${e}`)}l=crypto.randomUUID(),c={sdk:f(t),code:await n.text()},i.setAttribute(`sandbox`,`allow-scripts allow-forms`),j(),i.src=h(e)}function P(e){let t=typeof e?.url==`string`?e.url:``;if(!t)return``;try{let e=new URL(t);return e.hostname?` for ${e.hostname}`:``}catch{return``}}function F(e){switch(e?.type){case`airglow:openTab`:return`Airglow app "${t}" wants to open a browser tab${P(e)}. Allow this action?`;case`airglow:openWindow`:return`Airglow app "${t}" wants to open a browser window${P(e)}. Allow this action?`;case`airglow:identity:launchWebAuthFlow`:return`Airglow app "${t}" wants to open an authentication window${P(e)}. Allow this action?`;default:return null}}function I(e,n){let r=F(n);if(r&&!window.confirm(r)){e?.postMessage({_airglow_response:!0,_callId:n._callId,error:`User approval is required for this action`,code:`RUNTIME_USER_APPROVAL_DENIED`},`*`);return}let i={...n,_appId:t,...r?{[b]:!0}:{}};chrome.runtime.sendMessage(i,t=>{let r=chrome.runtime.lastError?{error:chrome.runtime.lastError.message||`Chrome runtime message failed`,code:`CHROME_RUNTIME_ERROR`}:t||{};e?.postMessage({_airglow_response:!0,_callId:n._callId,...r},`*`)})}window.addEventListener(`message`,e=>{if(!i||e.source!==i.contentWindow)return;let n=e.source,r=e.data;if(r?._airglow_ui_ready&&r.appId===t){document.body.dataset.airglowAppUiReady=`true`;return}if(r?._airglow_app_error&&r.appId===t){s||A(`App '${t}' hit an unhandled error`,`The app iframe is still isolated. You can reload only this app without restarting the extension.`,r);return}r?._airglow&&I(n,r)}),i=document.createElement(`iframe`),i.style.cssText=`position:fixed;inset:0;width:100%;height:100%;border:none;`,i.setAttribute(`sandbox`,`allow-scripts allow-same-origin allow-forms`),i.setAttribute(`allow`,`clipboard-read; clipboard-write`),i.onload=()=>{o=0,D(),document.getElementById(`loading`)?.remove(),c&&i?.contentWindow&&(i.contentWindow.postMessage({type:`airglow:ui:run`,appId:t,nonce:l,...c},`*`),c=null)},i.onerror=()=>{A(`App '${t}' failed to load`,`The browser reported an iframe load error. Reloading this app iframe should recover after a transient server or network issue.`)},document.body.appendChild(i),M(!1)}function E(e){let t=document.createElement(`div`);return t.textContent=e||``,t.innerHTML}