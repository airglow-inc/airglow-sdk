...
`}var f=`__app_sources`,p=`__app_manifests`,m=`__dev_server_online`,h=12e3,g=8e3,_=[1e3,3e3],v=[500,1500],y=new URLSearchParams(window.location.search),b=y.get(`app`);b?(x(b),C(b)):document.getElementById(`loading`).textContent=`Missing app parameter`;function x(e){chrome.runtime.sendMessage({type:`airglow:track-ui-page-opened`,appId:e},()=>{chrome.runtime.lastError})}function S(e){document.body.innerHTML=``;let t=`#b91c1c`,n=`#1c1917`,r=document.createElement(`div`);r.id=`airglow-app-offline`,r.style.cssText=`position:fixed;inset:0;display:flex;align-items:center;justify-content:center;padding:24px;background:#f5f5f4;color:${n};`,r.innerHTML=`
  ...
  `,document.body.appendChild(r),document.getElementById(`airglow-offline-retry`)?.addEventListener(`click`,()=>location.reload());let i=e=>{m in e&&e[m].newValue===!0&&(chrome.storage.local.onChanged.removeListener(i),location.reload())};chrome.storage.local.onChanged.addListener(i)}async function C(e){let t=await chrome.storage.local.get([m,f]),n=(t[f]||{})[e];if(t[m]===!1&&(!n||n.type===`local`)){S(e);return}if(n?.url){w(e,n);return}for(let t=0;t<5;t++){t>0&&await new Promise(e=>setTimeout(e,1e3));let n=((await chrome.storage.local.get(f))[f]||{})[e];if(n?.url){w(e,n);return}}l.warn(`airglow`,`no source registered for app '${e}' after 5 attempts`),document.getElementById(`loading`).textContent=`App '${e}' not found. Is an app source reachable?`}function w(e,t){let n=t.url.replace(/\/+$/,``),r=null,i=null,a=0,o=!1,s=null,c=``;chrome.storage.local.get(p).then(t=>{let n=(Array.isArray(t[p])?t[p]:[]).find(t=>t?.id===e);n?.name&&(document.title=n.name)}).catch(e=>{l.warn(`airglow`,`cached manifest title lookup failed: ${e instanceof Error?e.message:String(e)}`)});function u(t=!1){let r=new URLSearchParams(y);return t&&r.set(`_airglow_reload`,String(Date.now())),`${n}/api/apps/${e}/ui?${r.toString()}`}function f(t=!1){let r=new URLSearchParams;t&&r.set(`_airglow_reload`,String(Date.now()));let i=r.toString();return`${n}/api/apps/${e}/ui-bundle${i?`?${i}`:``}`}function m(t=!1){let n=new URLSearchParams(y);return n.set(`app`,e),n.set(`nonce`,c),t&&n.set(`_airglow_reload`,String(Date.now())),chrome.runtime.getURL(`app-ui-sandbox.html?${n.toString()}`)}function b(e){return new Promise(t=>setTimeout(t,e))}function x(e){return e===408||e===409||e===425||e===429||e>=500&&e<=599}async function S(e){let t=``;for(let n=0;n<=v.length;n++)try{let r=await fetch(e,{signal:AbortSignal.timeout(g)});if(!r.ok&&x(r.status)&&n<v.length){t=`UI bundle request failed with HTTP ${r.status}`,await b(v[n]);continue}return r}catch(e){if(t=e instanceof Error?e.message:String(e),n<v.length){await b(v[n]);continue}throw Error(`UI bundle request failed after ${v.length+1} attempts: ${t}`)}throw Error(`UI bundle request failed: ${t}`)}function C(e){let t=document.getElementById(`loading`);t||(t=document.createElement(`div`),t.id=`loading`,document.body.appendChild(t)),t.textContent=e}function w(){i&&=(clearTimeout(i),null)}function E(){document.getElementById(`airglow-app-crash`)?.remove(),o=!1}function D(e){return[e.name?`${e.name}: ${e.message}`:e.message,e.filename?`${e.filename}:${e.lineno??0}:${e.colno??0}`:``,e.stack||``].filter(Boolean).join(`

`)}function O(e,t,n){o=!0,w();let r=document.getElementById(`airglow-app-crash`);r&&r.remove();let i=document.createElement(`div`);i.id=`airglow-app-crash`,i.style.cssText=[`position:fixed`,`inset:0`,`z-index:2147483647`,`display:flex`,`align-items:center`,`justify-content:center`,`padding:24px`,`background:#f5f5f4`,`color:#1c1917`,`font-family:system-ui,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif`].join(`;`);let a=n?D(n):``;i.innerHTML=`
      <div style="width:min(640px,100%);border:1px solid #e7e5e4;border-radius:8px;background:#fff;padding:20px;box-shadow:0 12px 32px rgba(28,25,23,.12)">
        ...
        <div style="font-size:20px;font-weight:650;line-height:1.25;margin-bottom:8px">${T(e)}</div>
        <div style="font-size:14px;line-height:1.5;color:#57534e;margin-bottom:16px">${T(t)}</div>
        ${a?`<pre style="max-height:220px;overflow:auto;white-space:pre-wrap;background:#fafaf9;border:1px solid #e7e5e4;border-radius:6px;padding:12px;font-size:12px;line-height:1.45;color:#44403c;margin-bottom:16px">${T(a)}</pre>`:``}
    ...
    `,document.body.appendChild(i),document.getElementById(`airglow-reload-app`)?.addEventListener(`click`,()=>A(!0)),document.getElementById(`airglow-reload-extension`)?.addEventListener(`click`,()=>chrome.runtime.reload())}function k(){w(),i=setTimeout(()=>{let t=_[a];if(t!==void 0){a++,C(`App did not load. Retrying ${a}/${_.length}...`),window.setTimeout(()=>A(!0),t);return}O(`App '${e}' did not finish loading`,`The UI iframe did not fire a load event. This is usually a dev server, network, or bundle problem. Reloading the app iframe is enough; you should not need to restart the whole extension.`)},h)}function A(n=!1){if(E(),C(`Loading...`),r){if(t.type===`cloud`){w(),j(n).catch(t=>{O(`App '${e}' failed to load`,t instanceof Error?t.message:String(t))});return}r.setAttribute(`sandbox`,`allow-scripts allow-same-origin allow-forms`),r.src=u(n),k()}}async function j(t=!1){if(!r)return;let n=await S(f(t));if(!n.ok){let e=`${n.status}`;try{let t=await n.json();t?.error&&(e=typeof t.error==`string`?t.error:JSON.stringify(t.error))}catch{}throw Error(`UI bundle request failed: ${e}`)}c=crypto.randomUUID(),s={sdk:d(e,`app_ui`),code:await n.text()},r.setAttribute(`sandbox`,`allow-scripts allow-forms`),k(),r.src=m(t)}window.addEventListener(`message`,t=>{if(!r||t.source!==r.contentWindow)return;let n=t.source,i=t.data;if(i?._airglow_app_error&&i.appId===e){o||O(`App '${e}' hit an unhandled error`,`The app iframe is still isolated. You can reload only this app without restarting the extension.`,i);return}if(!i?._airglow)return;let a={...i,_appId:e};chrome.runtime.sendMessage(a,e=>{let t=chrome.runtime.lastError?{error:chrome.runtime.lastError.message||`Chrome runtime message failed`,code:`CHROME_RUNTIME_ERROR`}:e||{};n?.postMessage({_airglow_response:!0,_callId:i._callId,...t},`*`)})}),r=document.createElement(`iframe`),r.style.cssText=`position:fixed;inset:0;width:100%;height:100%;border:none;`,r.setAttribute(`sandbox`,`allow-scripts allow-same-origin allow-forms`),r.setAttribute(`allow`,`clipboard-read; clipboard-write`),r.onload=()=>{a=0,w(),document.getElementById(`loading`)?.remove(),s&&r?.contentWindow&&(r.contentWindow.postMessage({type:`airglow:ui:run`,appId:e,nonce:c,...s},`*`),s=null)},r.onerror=()=>{O(`App '${e}' failed to load`,`The browser reported an iframe load error. Reloading this app iframe should recover after a transient server or network issue.`)},document.body.appendChild(r),A(!1)}function T(e){let t=document.createElement(`div`);return t.textContent=e||``,t.innerHTML}